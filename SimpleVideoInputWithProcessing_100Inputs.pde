...
void draw() {
  ...
  fill(0);
  rect(0,0,320*something, 640);
  
  fill(0);
  rect(640-320*something,0,320*something,480);
...
void oscEvent(OscMessage Message) {
 if (Message.checkAddrPattern("/wek/outputs")==true) {
     if(Message.checkTypetag("f")) { // looking for 1 control value
        float MessageNumber = Message.get(0).floatValue();
        something = MessageNumber;
